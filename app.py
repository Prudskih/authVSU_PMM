...
# Демо-пользователи (создаются один раз, потом сохраняются в data/users.json)
...
def files():
    ...
    user_files = file_service.list_files(username=username, user_role=role)
...
def upload_pdf():
    ...
    try:
        safe_name = file_service.save_pdf(file, session['username'])
        flash(f"Файл {safe_name} загружен и защищён!", "success")
    except Exception:
...
@app.route('/view_pdf/<safe_name>')
def view_pdf(safe_name):
    if 'username' not in session:
        return redirect(url_for('login'))
    ...
    if not file_service.can_access(safe_name=safe_name, username=username, user_role=role):
        flash("Доступ запрещён.", "error")
        return redirect(url_for('files'))

    pdf_data = file_service.load_pdf(safe_name=safe_name)
    if pdf_data is None:
